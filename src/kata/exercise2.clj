...
(testing "Top 3 rich customers"
  ...
  ; Create a sequence with top 3 rich customers
  (let [top-customers
        (take 3 (sort #(> (:budget %1) (:budget %2)) (:customers mall)))]
...
(testing "Distinct age"
  ...
  (let [customer-ages
        (set (map :age (:customers mall)))]

    (is (= (count customer-ages) 9))
    (is (= customer-ages #{22, 27, 28, 26, 32, 35, 21, 36, 38}))))

(testing "Items customers want to buy"

  ; Create a sequence with items' names stored in :wants-to-buy
  (let [wanted-items
        (map :name (mapcat :wants-to-buy (:customers mall)))]
